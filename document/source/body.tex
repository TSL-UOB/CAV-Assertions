...
\begin{itemize}
    ...
    %
    Equation~\ref{DS_equation_rule126} is a regression formula derived from this table to estimate the stopping distance ($m$) given the vehicle speed ($mph$).\todo{``in $m$'' to distinguish $m$ from $v$?} 
...
The assertion checks whether a Safe Distance Ahead existed for an AV performing an overtake manoeuvre on a single-lane road, as it began to cross the road centre line at the start of the manoeuvre. 
%
...
\todo{reviewed to here by Kerstin :)}
...
At present, we are not in a position to perform assertion checking for this kind of safety property as an independent validation exercise %by means of an independent assertion-based oracle
