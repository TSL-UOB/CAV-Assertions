...
In our experimental study of runtime monitoring we developed an assertion monitor for checking safe stopping distance during the overtake. 
%
...
% GC Notes:
% In this example, certain profiles perform better?
% IT is possible to overtake in B/C
% Even distribution of overtaking situations
% Fix profile, move horizontal 65m acceptable (near miss so rejection reasonable), but if you reject 82m gap this impacts performance
% Any opportunity in zone D unaccptable, diagram can be sued to select profile
% Risk Zones in Fig7
% "Performance Requirement/Margin delta" -> replace opp window
% Aggressive may put others at risk, cannot always be drive like this
