...
%
To demonstrate the use of assertions at runtime, in the absence of a roadworthy AV controller, we used manual video analysis in combination with a pre-trained object classifier to extract data from a dashboard video camera of a dangerously overtaking vehicle.\footnote{Link to video used in analysis: \url{https://youtu.be/Gzi4X7WZkRI} time period 01:30 to 01:45 minutes.} \todo[inline]{indicate start-stop timing in video, from min:sec to min:sec}
...
\noindent \textbf{iv)} the AV should not enter the DS$_{VBP}$ and 

\noindent \textbf{v)} there should be no intersection between the DS$_{AV}$ and the DS$_{VBP}$. 
...
%
The separation distances between the AV and the other vehicles can be used as a safety performance indicator for the overtaking manoeuvre. For this to be determined we need to calculate the longitudinal and lateral separation distances between the other vehicles from the AV.
%
...
%
Third, the pull out $\beta$ and cut in $\theta$ angles of the AV are calculated using the change in lateral distance and change in the longitudinal distance measured. Fourth, as there is no vehicle speed associated with the video, we based the DS calculation on the worst case scenario; that is all vehicles were traveling at the speed limit of the road. 
%
...
&  &  &  & \\
VBP outside DS$_{AV}$ & PASS & PASS & FAIL\\
%
OV outside DS$_{AV}$ & PASS & FAIL & FAIL\\
%
AV outside DS$_{OV}$  & PASS & FAIL & FAIL\\
%
