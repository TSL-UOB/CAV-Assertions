...
An analysis for overtaking performance can be observed in the simulation results presented in Section~\ref{sim_case_study} by comparing the required SDA to the actual distance ahead of the Oncoming Vehicle. 
% as listed in Table~\ref{Overtaking_Profiles}. 
...
From the point of view of performance, the AV should make best use of the available gaps to overtake 
% without violating the safety property, 
and may need to change its driving profile in order to do so. 
%
...
\begin{figure}
    \centering
    \includegraphics[width=8.5cm]{\pathToOtherFiles/figures/gaussian_ada.png}
...
%
In general the more aggressive the driving style selected the closer to the unsafe boundary the AV will  be and hence the higher the number of overtaking opportunities that can be taken. 
%
...
%
Zones C and D describe situations where the required safe distance ahead is getting progressively larger and represent increasing levels of safety. 
%
...
% cut this???
% There is also evidence of this trade-off between performance and safety, while the AV should complete the overtake maneuver ``as soon as you can'' (higher performance) but ``do not cut in'' (higher safety) 
%
...
We also present the idea of using UKHC-based assertion checking methodology as a method to infer autonomous vehicle performance and have demonstrated this with an overtaking scenario using simulation results. We have demonstrated that a common UKHC assertion %-based oracle 
checking system
