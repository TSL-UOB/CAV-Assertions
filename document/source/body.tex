...
\noindent \textbf{ii)} the OV should not be in the AV$_{DS}$, %shouldn't this be DS$_{AV}$ ? 

\noindent \textbf{iii)} the AV should not be in the OV$_{DS}$, 
%shouldn't this be DS$_{OV}$ ?

\noindent \textbf{iv)} there should be no intersection between the AV$_{DS}$ and OV$_{DS}$.
% shouldn't the parameter and the vehicle identity be swapped over, as in the comments above?
...
When passing the VBP the assertion checks indicate that the overtake should have been aborted or if it was aborted then it should have been aborted earlier during passing the VBP stage as both the AV and the OV have entered each others danger spaces. Finally at the last stage of the overtake, the assertion checks suggest that the AV has attempted to cut in before it has completely overtaken the VBP, whilst still failing the assertions relating to the OV. However, the AV has passed the assertion of not being in the VBP$_{DS}$, % shouldn't the parameter and the vehicle identity be swapped over, as in the comments above?
 which suggests, given all of the other assertions have failed, that it has not overtaken the VBP before cutting in.
...
\end{table}
% shouldn't the parameter and the vehicle identities be swapped over, as in previous comments ?
