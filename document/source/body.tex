...
\end{equation}
    ...
    \item \emph{Danger Space} (DS) is an area projected forward in the direction of travel with a length equal to the stopping distance and a width equal to that of the vehicle. The overall formula for SDA incorporates the DS of the OV.\todo[inline]{adjust DS def in appendix to include width}
    %
...
\subsubsection{Simulation experiments}\todo[inline]{revise and tidy up this section}
...
\todo[inline]{A table note must be added to explain the use of intersection symbols as shorthand for a "does not overlap" relation between two polygons.}
% <<<<<<< HEAD
...
Considering this example from the point of view of performance, the AV must not violate the safety property but must overtake within a reasonable window of opportunity ($\delta$), i.e.\ taking any presented opportunity to overtake if safe to do so and if appropriate to the road network. \todo[inline]{clarify delta, which is measured in seconds and corresponds to TTC}
% >>>>>>> 73dcf5c238b93d3bf13283edf1ff00e8dcb5fd58
%
...
\end{itemize}
%
Here, $d_{OV}$ is the distance to the Oncoming Vehicle,\todo[inline]{is this the Distance Ahead in Fig.\ 2?} SDA is the required Safe Distance Ahead based on the model given in Appendix~\ref{appendix_a} and $\delta$ is the optimal window of opportunity for the vehicle to perform the manoeuvre. 
%
