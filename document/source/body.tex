...
% GC ADDED THIS
%
We illustrate the approach with two examples of assertion checking lane changes during an overtaking scenario. 
...
%
We adopt the terms scene, scenario and situation from~\cite{Ulbrich2015} throughout this paper. 
...
\end{figure}
  ...
  \todo[inline, color=green]{Have changed figure to include this}


  \begin{itemize}
      \item The \emph{Experiment Instantiation} subsystem generates test scenarios derived from the \emph{Experiment Specification} which may include test objectives or requirements (defined in natural language). The Experiment Instantiation subsystem identifies the \emph{Static} information about the environment (e.g. maps, road features) and \emph{Dynamic} descriptions of the behaviour required of simulated actors (vehicles, other road users, traffic lights, any third-party agents necessary to the test requirement). \todo[inline]{Why subsystem? Can we not just use the name of the respective box(es)?} \todo[inline, color=green]{Have updated this sentence}
      The experimental configuration determines the test case that will contribute coverage towards the assertion.\todo[inline]{Nothing said yet about coverage.}\todo[inline, color=green]{Will add another section at end of list}
      ...
      \item The \emph{Data Capture} subsystem records relevant simulation data about the driving situation of the AV, including actor positions and road network information, and at each time step of the simulation writes this information into the records of the database to be used for assertion checking.\todo[inline]{Should there not be a DB symbol (looks like a tin) to show the database we are sourcing?}\todo[inline, color=green]{Have added to the figure}
      ...
      \item The \emph{Outputs} are a a set of Pass/Fail declarations which can be used to annotate the simulation log or be recorded in a separate file if desired. \emph{Assertion coverage} with respect to the experiment specifications can also be monitored at the output. \todo[inline]{Should there not be a DB symbol to represent the assertion library, i.e.\ a set of SQL queries?} \todo[inline, color=green]{Have added the DB symbol in the figure}
  \end{itemize}
